...
const POSICOES = [
  'Goleiro',
  'Zagueiro Central', 'Zagueiro Direito', 'Zagueiro Esquerdo',
...
function Field({
  label, name, type = 'text', placeholder, value, onChange, required, maxLength, error, children,
}: {
  label: string; name?: string; type?: string; placeholder?: string
  value?: string; onChange?: (v: string) => void; required?: boolean
  maxLength?: number; error?: string; children?: React.ReactNode
}) {
  return (
...
function SelectField({ label, name, value, onChange, required, error, children }: {
...
function RouteComponent() {
  ...
  const tipoConta = isOlheiro ? 'OLHEIRO' : 'ATLETA'

  const SECTIONS = ["Sobre Você", isOlheiro ? "Dados Profissionais" : "Dados Esportivos", "Acesso"]
  ...
  const validarStep1 = () => {
    ...
    else if (nome.trim().split(' ').length < 2) errs.nome = 'Informe nome e sobrenome.'
    if (!dataNascimento) errs.dataNascimento = 'Data de nascimento obrigatória.'
    if (!cpf) errs.cpf = 'CPF obrigatório.'
    else if (onlyDigits(cpf).length !== 11) errs.cpf = 'CPF deve ter 11 dígitos.'
    if (!telefone) errs.telefone = 'Telefone obrigatório.'
    else if (onlyDigits(telefone).length < 10) errs.telefone = 'Telefone inválido.'
    if (cep && onlyDigits(cep).length !== 8) errs.cep = 'CEP deve ter 8 dígitos.'
  ...
  const avancarStep1 = (e: React.FormEvent) => {
    e.preventDefault()
    if (validarStep1()) setCurrentSection(STEPS.PROFISSIONAL_HISTORY)
  }

  const avancarStep2 = (e: React.FormEvent, pular = false) => {
    e.preventDefault()
    if (pular || !isOlheiro) { setCurrentSection(STEPS.ACCESS_FORM); return }
  ...
  const finalizarCadastro = async (e: React.FormEvent) => {
    ...
    try {
      await api.cadastrar({
        ...
        peDominante: peDominante || undefined,
        peso: peso || undefined,
        altura: altura || undefined,
        clubesAnteriores: isOlheiro
          ? (empresaOuClube ? `${empresaOuClube} | Região: ${regiaoAtuacao}` : undefined)
  ...
  const inputClass = 'py-3 px-5 border border-gray-200 rounded-md placeholder-gray-300 focus:outline-none focus:border-slate-400 focus:bg-slate-50 text-base w-full'
  const selectClass = 'py-3 px-5 border border-gray-200 rounded-md text-base w-full bg-white focus:outline-none focus:border-slate-400'
  ...
  const renderSwitch = () => {
    switch (currentSection) {
      ...
      default:
        return (
          <form onSubmit={avancarStep1} className='w-full md:min-w-[670px]' noValidate>
            <section className='shadow-lg rounded-lg mb-4 p-6 flex flex-col gap-4'>
              <div className='bg-sky-50 border border-sky-200 rounded-lg px-4 py-2 text-sm text-sky-800 font-semibold'>
                Cadastro de <span className='uppercase'>{isOlheiro ? 'Olheiro' : 'Atleta'}</span>
              </div>

              <Field label='Seu Nome' required error={errStep1.nome}>
                <input
                  type='text' name='nome' placeholder='Nome e Sobrenome'
                  value={nome} onChange={e => setNome(e.target.value)}
                  className={`${inputClass} ${errStep1.nome ? 'border-red-400 bg-red-50' : ''}`}
                />
              </Field>

              <Field label='Data de Nascimento' required error={errStep1.dataNascimento}>
                <input
                  type='date' name='dataNascimento' value={dataNascimento}
              ...
              <div className='grid grid-cols-2 gap-3'>
                ...
                <Field label='Telefone' required error={errStep1.telefone}>
                  <input
                    type='text' name='telefone' placeholder='(11) 99999-9999'
              ...
              <div className='grid grid-cols-2 gap-3'>
                <Field label='Estado' required error={errStep1.estado}>
                  <select
                    name='estado' value={estado} onChange={e => setEstado(e.target.value)}
                ...
                <Field label='Cidade' required error={errStep1.cidade}>
                  <input
                    type='text' name='cidade' value={cidade}
            ...
            <div className='flex justify-end'>
              <button type='submit' className='button primary rounded-full px-12'>Próximo</button>
            </div>
          </form>
        )
      ...
      case STEPS.PROFISSIONAL_HISTORY:
        return (
          <form onSubmit={e => avancarStep2(e, false)} className='w-full md:min-w-[670px]' noValidate>
            <section className='shadow-lg rounded-lg mb-4 p-6 flex flex-col gap-4'>
              {isOlheiro ? (
                <>
                  <p className='text-slate-500 text-sm'>Dados sobre sua atuação como olheiro.</p>

                  <Field label='Clube ou Empresa que Representa' required error={errStep2.empresaOuClube}>
                    <input
                      type='text' placeholder='Ex: Clube Atlético Mineiro'
                      value={empresaOuClube} onChange={e => setEmpresaOuClube(e.target.value)}
                      className={`${inputClass} ${errStep2.empresaOuClube ? 'border-red-400 bg-red-50' : ''}`}
                    />
                  </Field>

                  <Field label='Região de Atuação' required error={errStep2.regiaoAtuacao}>
                    <input
                      type='text' placeholder='Ex: São Paulo, Sudeste'
                      value={regiaoAtuacao} onChange={e => setRegiaoAtuacao(e.target.value)}
                  ...
                  <Field label='Anos de Experiência'>
                    <input
                      type='number' placeholder='Ex: 5' min={0} max={50}
                ...
                <>
                  <p className='text-slate-500 text-sm'>Esta seção é opcional. Você pode preencher depois nas configurações.</p>

                  <SelectField label='Posição que Joga' name='posicao' value={posicao} onChange={setPosicao} error={errStep2.posicao}>
                    <option value=''>Selecione uma posição</option>
                    {POSICOES.map(p => <option key={p} value={p}>{p}</option>)}
                  </SelectField>

                  <SelectField label='Pé Dominante' name='peDominante' value={peDominante} onChange={setPeDominante}>
                    <option value=''>Selecione</option>
                  ...
                  <div className='grid grid-cols-2 gap-3'>
                    <Field label='Peso (kg)'>
                      <input type='number' placeholder='Ex: 75' value={peso} onChange={e => setPeso(e.target.value)} className={inputClass} />
                    </Field>
                    <Field label='Altura (cm)'>
                      <input type='number' placeholder='Ex: 178' value={altura} onChange={e => setAltura(e.target.value)} className={inputClass} />
                    </Field>
                  </div>

                  <Field label='Clubes Anteriores'>
                    <input type='text' placeholder='Ex: São Paulo FC, Corinthians' value={clubesAnteriores} onChange={e => setClubesAnteriores(e.target.value)} className={inputClass} />
                  </Field>
                </>
              )}
            </section>
            <div className='flex justify-end items-center gap-4'>
              {!isOlheiro && (
                <button type='button' onClick={e => avancarStep2(e as any, true)} className='button primary-outlined border rounded-full px-8'>Pular</button>
              )}
              <button type='submit' className='button primary rounded-full px-12'>Próximo</button>
            </div>
          </form>
      ...
      case STEPS.ACCESS_FORM:
        return (
          <form onSubmit={finalizarCadastro} className='w-full md:min-w-[670px]' noValidate>
            <section className='shadow-lg rounded-lg mb-4 p-6 flex flex-col gap-4'>
              <Field label='E-Mail' required>
                <input type='email' name='email' placeholder='seu@email.com' value={email} onChange={e => setEmail(e.target.value)} className={inputClass} />
              </Field>
              <Field label='Senha' required>
                <input type='password' name='senha' placeholder='Mínimo 6 caracteres' value={senha} onChange={e => setSenha(e.target.value)} className={inputClass} />
              </Field>
              <Field label='Confirmar Senha' required>
                <input type='password' name='confirmarSenha' value={confirmarSenha} onChange={e => setConfirmarSenha(e.target.value)} className={inputClass} />
              </Field>
              {erro && <p className='text-red-600 text-sm'>{erro}</p>}
            </section>
            <div className='flex justify-end'>
              <button type='submit' className='button primary rounded-full px-12'>Criar Conta</button>
            </div>
          </form>
        )
    }
    ...
    <div className='page-noscroll flex flex-col'>
      <div className='max-w-[1024px] mx-auto content grow-1 flex'>
        <section className='flex flex-col p-4 justify-stretch w-full'>
          <div className='mt-16'>
            <Link className='text-sm primary-outlined inline-flex items-center gap-2 transition text-sky-600 hover:text-sky-900' to='/login'>
