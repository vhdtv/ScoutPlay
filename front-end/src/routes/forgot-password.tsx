      ...
      <section className="content grow-1 min-h-0 flex flex-row">
        ...
        <div className='shrink-0 sm:grow-0 flex flex-col flex-wrap items-center justify-center'>
          <div className='border-b-2 border-gray-200 p-3 min-w-xl grow-1 flex flex-col pt-12'>
            <h3 className='text-sky-950 font-extrabold text-2xl mb-2'>Recuperar a senha</h3>
            <p className='text-slate-500 text-sm mb-5'>
              Informe seu e-mail cadastrado e enviaremos uma nova senha temporária.
            </p>

            {status === 'success' ? (
              <div className='flex flex-col items-center gap-4 py-8 text-center'>
                <span className='material-symbols-outlined text-green-500' style={{ fontSize: '3rem' }}>mark_email_read</span>
                <p className='text-green-700 font-semibold text-lg'>E-mail enviado!</p>
                <p className='text-slate-500 text-sm'>Verifique sua caixa de entrada e use a nova senha para entrar.</p>
                <Link to='/login' className='mt-4 button primary rounded-full px-6'>Ir para o login</Link>
              </div>
            ) : (
              <form action={sendRecoveryEmail} className='grow-1 flex flex-col gap-4'>
          ...
          <div className='text-center py-12 px-3 w-full'>
            ...
            <Link className="text-underline text-sky-700 underline underline-offset-8" to="/login">
