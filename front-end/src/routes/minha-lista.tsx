...
function AtletaRow({ atleta, onRemover }: { atleta: AtletaCardDTO; onRemover: (u: string) => void }) {
  return (
    <div className='flex items-center gap-4 p-3 rounded-xl bg-white border border-slate-100 shadow-sm hover:shadow transition'>
      ...
      <div className='flex-1 min-w-0'>
        ...
        <div className='flex items-center gap-2 mt-0.5 flex-wrap'>
          ...
          {atleta.posicao && (
            <span className='text-xs px-2 py-0.5 rounded-full bg-sky-50 text-sky-700'>{atleta.posicao}</span>
          )}
          {atleta.idade != null && (
            <span className='text-xs text-slate-500'>{atleta.idade} anos</span>
          )}
          {atleta.peDominante && (
            <span className='text-xs text-slate-500'>Pé {atleta.peDominante.toLowerCase()}</span>
          )}
        </div>
        {atleta.clubesAnteriores && (
          <p className='text-xs text-slate-400 truncate mt-0.5'>Clubes: {atleta.clubesAnteriores}</p>
        )}
      </div>
    ...
    <div className='min-h-screen flex flex-col bg-slate-50'>
      ...
      <div className='container mx-auto px-4 py-8 max-w-3xl flex-1'>
        <div className='flex items-center justify-between mb-6'>
          ...
          <span className='text-sm text-slate-400'>{lista.length} atleta{lista.length !== 1 ? 's' : ''}</span>
        </div>
        ...
        {lista.length === 0 ? (
          <div className='flex flex-col items-center justify-center h-48 text-slate-400 gap-3'>
            ...
            <Link to='/atletas' className='text-sm text-sky-600 hover:underline font-medium'>Buscar atletas</Link>
          </div>
        ) : (
