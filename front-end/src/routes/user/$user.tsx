...
function RouteComponent() {
	...
	useEffect(() => {
		...
		setIsResponsavel(api.isResponsavel());
		if (isAtleta) {
			api
				.listarAvaliacoes(user.username)
				.then(setAvaliacoes)
				.catch(() => {});
		}
	}, [isAtleta, user.username]);
	...
	const vincularComoResponsavel = async () => {
		if (vinculando) return;
		setVinculando(true);
		try {
			await api.vincularComoResponsavelAConta(user.username);
			window.alert("Vínculo de responsável registrado com sucesso.");
		} catch (error) {
			window.alert(
				error instanceof Error
					? error.message
					: "Não foi possível registrar o vínculo.",
			);
		} finally {
			setVinculando(false);
		}
	...
	const enviarAvaliacao = async () => {
		...
		try {
			const nova = await api.avaliarAtleta(
				user.username,
				nota,
	...
	const handleDeletePost = (url: string) =>
		setPosts((prev) => prev.filter((p) => p.url !== url));
	...
	const toggleSeguir = async () => {
		...
		if (souSeguidor) {
			...
			if (ok) {
				setSouSeguidor(false);
				setSeguidores((n: number) => Math.max(0, n - 1));
			}
		} else {
			const ok = await api.seguir(user.username);
			if (ok) {
				setSouSeguidor(true);
				setSeguidores((n: number) => n + 1);
			}
		}
		...
		<div className="min-h-screen flex flex-col bg-slate-50">
				...
				<div className="relative -mt-12 flex flex-col sm:flex-row sm:items-end gap-4 pb-4">
					...
					<div className="flex flex-col gap-0 flex-1">
						<div className="flex flex-wrap items-center gap-2 pb-4">
							...
							{user.idade != null && (
								<span className="text-slate-300 text-md/2">
									· {user.idade} anos
								</span>
							)}
						</div>
						<div className="flex flex-wrap gap-1.5">
							{(user.tipoConta as any[])?.map((t: string) => (
								<span
									key={t}
									className="text-xs px-2.5 py-0.5 rounded-full bg-sky-100 text-sky-700 font-semibold"
								>
					...
					<div className="flex items-center gap-4 ml-auto shrink-0">
						...
						{user.souEu ? (
							<Link
								to="/settings"
								className="px-5 py-2 rounded-full border border-sky-600 text-sky-600 hover:bg-sky-600 hover:text-white text-sm font-semibold transition"
							>
								Editar perfil
							</Link>
						) : (
								...
								{isOlheiro && isAtleta && (
									...
									>
										...
										<span className="hidden sm:inline">
											{naShortlist ? "Na lista" : "Minha lista"}
										</span>
									</button>
								)}
								{isResponsavel &&
									...
									user.idade < 18 && (
										<button
											onClick={vincularComoResponsavel}
											disabled={vinculando}
											className="px-4 py-2 rounded-full text-sm font-semibold border border-emerald-500 text-emerald-700 hover:bg-emerald-50 disabled:opacity-50"
										>
											{vinculando ? "Vinculando..." : "Sou responsável"}
										</button>
									)}
							</>
						)}
			...
			<div className="container mx-auto px-4 grid grid-cols-12 gap-4 pb-10 min-h-[100vh]">
				...
				<div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
					{(dp.posicao || dp.altura || dp.peso || dp.peDominante) && (
						<div className="bg-white rounded-xl shadow p-4">
							...
							<div className="flex flex-wrap gap-2">
								...
								<StatCard
									label="Altura"
									value={dp.altura ? `${dp.altura} cm` : null}
								/>
								<StatCard
									label="Peso"
									value={dp.peso ? `${dp.peso} kg` : null}
								/>
					...
					{dp.RESPONSAVEL &&
						Array.isArray(dp.RESPONSAVEL) &&
						dp.RESPONSAVEL.length > 0 && (
							<div className="bg-white rounded-xl shadow p-4">
								<h2 className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-3">
									Responsável
								</h2>
							...
							{isOlheiro && !user.souEu && (
								<div className="mb-5 p-3 bg-slate-50 rounded-xl border border-slate-200">
									...
									<div className="flex items-center gap-3 mb-2">
										<label className="text-sm text-slate-600 w-16 shrink-0">
											Nota (1-10)
										</label>
										<input
											type="range"
											...
											className="flex-1"
										/>
										<span className="text-sm font-bold text-sky-700 w-6 text-right">
											{nota}
										</span>
									</div>
									<textarea
										...
										<div key={av.id} className="flex gap-3 items-start">
											...
											<div className="flex-1">
												<div className="flex items-center gap-2">
													...
													<span className="ml-auto text-sm font-bold text-sky-700">
														{av.nota}/10
													</span>
												</div>
												{av.comentario && (
