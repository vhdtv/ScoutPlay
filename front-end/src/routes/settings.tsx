...
function RouteComponent() {
	...
	const salvarPerfil = async (form: FormData) => {
		...
		try {
			...
			setUsuario((u) => ({
				...u,
				nome: nome ?? u.nome,
				sobrenome: sobrenome ?? u.sobrenome,
	...
	const salvarEsportivos = async (form: FormData) => {
		...
		for (const key of keys) {
			...
			if (val?.trim()) await api.definirDetalhePerfil(key, val.trim());
		}
		showSaved();
	};
			...
			<div className="container mx-auto px-4 py-6 flex flex-col lg:flex-row gap-4 grow">
				...
				<aside className="lg:w-60 shrink-0">
					<div className="bg-white rounded-xl shadow p-2">
						<p className="text-xs font-bold uppercase text-slate-400 px-3 py-2 tracking-wide">
							Configurações
						</p>
						{NAV_ITEMS.map((item) => (
							<button
				...
				<div className="flex-1 bg-white rounded-xl shadow p-6 min-h-96">
					...
					{section === "perfil" && (
						<form action={salvarPerfil} className="flex flex-col gap-5">
							...
							<button
								type="submit"
								className="self-start px-6 py-2 rounded-full bg-sky-600 hover:bg-sky-800 text-white text-sm font-semibold cursor-pointer transition"
							>
								Salvar
							</button>
						</form>
					...
					{section === "esportivos" && (
						<form action={salvarEsportivos} className="flex flex-col gap-5">
							...
							<div className="grid grid-cols-2 gap-4">
								<Input
									defaultValue={dp.peso ?? ""}
									fieldName="peso"
									label="Peso (kg)"
								/>
								<Input
									defaultValue={dp.altura ?? ""}
									fieldName="altura"
									label="Altura (cm)"
								/>
							</div>
							<Input
								defaultValue={dp.clubesAnteriores ?? ""}
								fieldName="clubesAnteriores"
					...
					{section === "conta" && (
						<div className="flex flex-col gap-4">
							...
							<p className="text-slate-500 text-sm">
								Gerenciamento de conta estará disponível em breve.
							</p>
						</div>
					)}
				</div>
