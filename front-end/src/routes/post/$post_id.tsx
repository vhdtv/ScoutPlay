...
function LikeCommentButton({
	...
	return (
		<button
			onClick={toggle}
			className={`flex items-center gap-1 text-xs transition cursor-pointer ${curtido ? "text-sky-600 font-semibold" : "text-slate-500 hover:text-sky-600"}`}
		>
			<span
	...
	return (
		<div className="flex gap-1 mt-1 ms-8">
			...
			<button
				...
				className="flex items-center p-1.5 bg-white hover:bg-sky-700 hover:text-white transition cursor-pointer rounded-full outline-none disabled:opacity-40"
			>
				<span
					className="material-symbols-outlined"
					style={{ fontSize: "1rem" }}
...
function CommentItem({
	...
	const [respostas, setRespostas] = useState<CommentDTO[]>(
		comment.respostas ?? [],
	);
	...
	return (
		<div className={depth > 0 ? "ms-6 border-l-2 border-slate-200 ps-2" : ""}>
			<div className="flex gap-2 px-2 py-1 items-start">
				<Link
					to="/user/$user"
					params={{ user: comment.por.username }}
					className="shrink-0"
				>
				...
				<div className="flex flex-col grow min-w-0">
					...
					<div className="flex gap-3 mt-1">
						<LikeCommentButton
							...
							postId={postId}
							count={comment.quantidadeLike}
							liked={comment.euCurti}
						/>
						{depth === 0 && (
							<button
								onClick={() => setRespondendo((v) => !v)}
								className="text-xs text-slate-500 hover:text-sky-600 transition cursor-pointer"
							>
								Responder
							</button>
						)}
...
function LikeButton({
	...
	postUrl: string;
	contagemAtualDeLike: number;
	usuarioJaDeuLike: boolean;
}) {
	const [contadorLike, definirContadorLike] = useState(contagemAtualDeLike);
	const [likeDoUsuarioLogado, definirLikeDoUsuarioLogado] =
		useState(usuarioJaDeuLike);
	...
	return (
		<>
			{!likeDoUsuarioLogado ? (
				<button
					onClick={enviarGostei}
					className="relative transition focus:backdrop-blur-sm focus:outline-2 outline-offset-3 outline-neutral-100 border-1 border-slate-100 p-5 flex flex-col items-center justify-center rounded-full cursor-pointer text-slate-100 hover:bg-white/50 hover:border-white/0 hover:text-slate-950 hover:backdrop-blur-sm focus:bg-white/50 focus:border-white/0 focus:text-slate-950"
				>
					<span
						className="-translate-y-1 material-symbols-outlined"
						style={{ fontSize: "1rem", lineHeight: "1em" }}
					>
						{" "}
						thumb_up{" "}
					</span>
					<span
						className="-translate-y-0.5 absolute bottom-2"
						style={{ fontSize: ".8rem" }}
					>
						{" "}
						{contadorLike}
				...
				<button
					onClick={enviarNaoGostei}
					className="relative transition focus:backdrop-blur-sm focus:outline-2 outline-offset-3 outline-neutral-100 border-1 border-slate-100 p-5 flex flex-col items-center justify-center rounded-full cursor-pointer bg-white/50 border-white/0 text-slate-950 backdrop-blur-sm hover:bg-white/20 hover:text-white focus:bg-white/20 focus:text-white"
				>
					<span
						className="-translate-y-1 material-symbols-outlined"
						style={{ fontSize: "1rem", lineHeight: "1em" }}
...
export function PostMediaComponent({
	post,
	className,
	showLike = true,
}: {
		...
		<div className={`relative w-full h-full ${className ?? ""}`}>
			<div className="absolute w-full h-full flex justify-start items-end overflow-hidden gap-2">
				...
				{showLike && (
					<div className="shrink-0 bottom-16 start-4 absolute z-1">
						<LikeButton
							contagemAtualDeLike={post.interacoes?.quantidadeLike ?? 0}
							postUrl={post.url}
							usuarioJaDeuLike={post.interacoes?.deuLike ?? false}
						/>
					</div>
				)}
				<div className="grow-1 overflow-hidden backdrop-blur-xs">
					<div className="w-full p-3 ps-4 flex gap-4 items-center justify-start snap-x scroll-pl-6 overflow-x-auto no-scrollbar">
						{post.interacoes?.destaques?.map((destaque, index) => (
							<span
								key={index}
								className="shrink-0 p-1 rounded-full px-4 flex items-center border border-white text-white relative"
							>
								#{destaque.texto}{" "}
			...
			{post.media.mimeType.startsWith("video") ? (
				<video
					...
					autoPlay={true}
					muted
					src={api.obterMidia(post.media.src)}
				/>
			) : (
				<img
					src={api.obterMidia(post.media.src)}
	...
	return (
		<div className="min-h-screen flex flex-col bg-slate-900">
			...
			<div className="flex-1 flex items-stretch justify-center px-2 py-4">
				<div
					className="relative w-full max-w-5xl shadow-2xl flex flex-col md:flex-row rounded-xl overflow-hidden"
				...
				>
					...
					{/* Comments panel */}
					<div
						className="w-full md:w-80 lg:w-96 bg-white flex flex-col"
					...
					>
						...
						<div className="flex-1 overflow-y-auto py-2 min-h-0">
							...
							{comentarios.map((c, i) => (
								<CommentItem key={c.id ?? i} comment={c} postId={post.url} />
							))}
							<div ref={bottomRef} />
						</div>
