...
const MENSAGEM_BOAS_VINDAS: MensagemDTO = {
	autor: "IA",
	texto:
		'Olá! Sou o Especialista ScoutPlay. Posso te ajudar a buscar atletas por posição, pé dominante ou clube, comparar jogadores cadastrados e responder perguntas táticas sobre futebol. Experimente perguntar: "Quais atacantes estão cadastrados?" ou "Me fale sobre os goleiros disponíveis."',
};

function AIChatComponent() {
	const [aberto, setAberto] = useState(true);
	const [mensagens, definirMensagens] = useState<MensagemDTO[]>([
	...
	const enviar = async () => {
		...
		try {
			...
			definirMensagens((prev) => [
				...prev,
				{ autor: "IA", texto: "Serviço de IA temporariamente indisponível." },
			]);
		} finally {
			setCarregando(false);
		}
					...
					<div className="px-4 py-3 bg-gradient-to-r from-sky-600 to-sky-500 flex items-center justify-between">
						<div className="flex items-center gap-2">
							...
							<span className="font-bold text-white text-sm">
								O Espec<span className="text-sky-200">IA</span>lista
							</span>
						</div>
						<button
					...
					<div className="h-64 overflow-y-auto p-3 flex flex-col gap-2 bg-slate-50">
						{mensagens.map((m, i) =>
							m.autor === "USUARIO" ? (
								<div key={i} className="flex justify-end">
									<span className="p-2 px-3 rounded-2xl rounded-br-sm bg-sky-600 text-white max-w-[85%] text-sm">
										{m.texto}
									</span>
								</div>
							) : (
								<div key={i} className="flex justify-start">
									<span className="p-2 px-3 rounded-2xl rounded-bl-sm bg-white border border-slate-200 shadow-sm text-slate-700 max-w-[85%] text-sm">
										{m.texto}
									</span>
								</div>
							),
						...
						{carregando && (
							<div className="flex justify-start">
								<span className="p-2 px-3 rounded-2xl rounded-bl-sm bg-white border border-slate-200 text-slate-400 text-sm italic">
									Digitando...
								</span>
							</div>
						)}
...
function BotaoSeguir({
	...
	return seguindo ? (
		<button
			onClick={onParar}
			disabled={loading}
			className="text-xs font-semibold px-3 py-0.5 rounded-full border border-slate-300 text-slate-600 hover:bg-slate-100 transition cursor-pointer disabled:opacity-40"
		>
			Seguindo
		</button>
	) : (
		<button
			onClick={onSeguir}
			disabled={loading}
			className="text-xs font-semibold px-3 py-0.5 rounded-full bg-sky-600 text-white hover:bg-sky-700 transition cursor-pointer disabled:opacity-40"
		>
			Seguir
		</button>
	);
	...
	return (
		<article className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow">
			...
			<div className="flex items-center gap-3 px-4 py-3 border-b border-slate-50">
				<Link
					to="/user/$user"
					params={{ user: post.autor.username }}
					className="shrink-0"
				>
				...
				<Link
					to="/user/$user"
					params={{ user: post.autor.username }}
					className="flex flex-col min-w-0 flex-1 hover:opacity-80 transition"
				>
				...
				<div className="flex items-center gap-3">
					...
					>
						<span
							...
							style={{
								fontSize: "1.3rem",
								fontVariationSettings: deuLike ? "'FILL' 1" : "'FILL' 0",
							}}
						>
							thumb_up
						</span>
...
function DialogPostCreateComponent({
	...
	return (
		<div
			onClick={onClose}
			className="fixed flex items-center justify-center top-0 start-0 end-0 bottom-0 z-50 bg-slate-900/60 backdrop-blur-sm"
		>
			<form
				action={criarPost}
				onClick={(e) => e.stopPropagation()}
				className="h-128 flex gap-2 w-full max-w-256 bg-white rounded-2xl overflow-hidden mx-4 shadow-2xl"
			>
				<div
					style={{
				...
				<div className="w-3/4 text-start flex flex-col gap-4 p-6">
					...
					<button
						type="submit"
						className="self-start px-6 py-2 rounded-full bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold cursor-pointer transition"
					>
						Publicar
					</button>
				</div>
...
export function RouteComponent() {
	...
	const obterPostsParaFeed = async (page = 0) => {
		...
		setSeguindoSet((prev) => {
			...
			data.forEach((p) => {
				if (p.metadados?.segueConta) next.add(p.autor.username);
			});
			return next;
		});
	};
	...
	const filteredPosts = useMemo(
		() =>
			posts.filter((post) => {
				const posicaoOk =
					!filtros.posicao || post.autor.posicao === filtros.posicao;
				const tipoOk =
					!filtros.tipoConta ||
					(post.autor.tipoConta?.includes(filtros.tipoConta) ?? false);
							...
							<div className="flex flex-col gap-3">
								<div className="flex flex-col gap-1">
									...
									<select
										...
										onChange={(e) =>
											setFiltros((f) => ({ ...f, posicao: e.target.value }))
										}
								...
								<div className="flex flex-col gap-1">
									...
									<select
										...
										onChange={(e) =>
											setFiltros((f) => ({ ...f, tipoConta: e.target.value }))
										}
										className="w-full py-1.5 px-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-sky-400 bg-white"
									>
				...
				<main className="col-span-12 lg:col-span-6 flex flex-col gap-4">
					...
					{filteredPosts.map((post) => (
						<PostFeedComponent
							...
							onSeguir={() =>
								setSeguindoSet(
									(prev) => new Set([...prev, post.autor.username]),
								)
							}
							onParar={() =>
								setSeguindoSet((prev) => {
									const s = new Set(prev);
									s.delete(post.autor.username);
									return s;
								})
							}
						/>
