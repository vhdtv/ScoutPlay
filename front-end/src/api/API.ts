...
export default class {
	...
	validarSessao = async (): Promise<boolean> => {
		try {
			const request = await fetch(`${this.BACKEND_ENDPOINT}/session`, {
				method: "GET",
				credentials: "include",
			});
	...
	redefinirSenha = async (
		...
		const request = await fetch(`${this.BACKEND_ENDPOINT}/reset-password`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
	...
	vincularComoResponsavelAConta = async (
		nomeUsuario: string,
	): Promise<boolean> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/user/${nomeUsuario}/responsavel`,
			{
				method: "POST",
				credentials: "include",
			},
		...
		if (!request.ok) {
			const json = await request.json().catch(() => ({}));
			throw new Error(json.message ?? "Não foi possível vincular o atleta");
		}
		return true;
	};
	...
	criarPost = async ({
		...
		arquivo,
		descricao,
	}: PostDataInputDTO): Promise<PostDataOutputDTO> => {
	...
	obterPost = async (postId: string): Promise<PostDetailsDTO> => {
		const backendRequest = await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}`,
			{
				method: "GET",
				credentials: "include",
			},
	...
	darDislikeEmPost = async (postId: string): Promise<boolean> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/dislike`,
			{
				method: "POST",
				credentials: "include",
				headers: {
	...
	obterComentarios = async (postId: UUID): Promise<CommentDTO[]> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/comments`,
			{
				credentials: "include",
				headers: { "Content-Type": "application/json" },
			},
	...
	enviarComentario = async (
		...
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/comment`,
			{
				method: "POST",
				credentials: "include",
				headers: { "Content-Type": "application/json" },
	...
	responderComentario = async (
		...
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/comment/${commentId}/reply`,
			{
				method: "POST",
				credentials: "include",
				headers: { "Content-Type": "application/json" },
	...
	curtirComentario = async (postId: UUID, commentId: string): Promise<void> => {
		await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/comment/${commentId}/like`,
			{
				method: "POST",
				credentials: "include",
			},
	...
	descurtirComentario = async (
		postId: UUID,
		commentId: string,
	): Promise<void> => {
		await fetch(
			`${this.BACKEND_ENDPOINT}/post/${postId}/comment/${commentId}/dislike`,
			{
				method: "POST",
				credentials: "include",
			},
	...
	seguir = async (nomeUsuario: string): Promise<boolean> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/user/${nomeUsuario}/seguir`,
			{
				method: "POST",
				credentials: "include",
			},
	...
	chatIA = async (prompt: string): Promise<MensagemDTO> => {
		...
		if (!json.success)
			throw new Error(json.message ?? "Serviço de IA indisponível");
	...
	obterPostsFeed = async ({
		page,
	}: {
		page: number;
	}): Promise<{ page: number; pageSize: number; data: PostDetailsDTO[] }> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/post/?page=${page}&size=10`,
			{
				method: "GET",
				credentials: "include",
			},
	...
	avaliarAtleta = async (
		...
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/user/${username}/avaliar`,
			{
				method: "POST",
				credentials: "include",
				headers: { "Content-Type": "application/json" },
	...
	listarAvaliacoes = async (username: string): Promise<AvaliacaoDTO[]> => {
		const request = await fetch(
			`${this.BACKEND_ENDPOINT}/user/${username}/avaliacoes`,
			{
				credentials: "include",
			},
		);
	...
	buscarAtletas = async (params: {
		...
		if (params.peDominante)
			url.searchParams.set("peDominante", params.peDominante);
	...
	isResponsavel = (): boolean =>
		this.obterTipoContaUsuario().includes("RESPONSAVEL");
}
