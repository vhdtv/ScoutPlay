  ...
  return (
    <div className='grid grid-cols-2 md:grid-cols-3 gap-1'>
      {posts.map((post, index) => (
        <PostThumbnail key={post.url ?? index} post={post} souEu={souEu} onDelete={onDelete} />
      ))}
    </div>
  )
}
...
function PostThumbnail({
  ...
  return (
    ...
    >
      ...
      <div className='absolute inset-0 opacity-0 group-hover:opacity-100 transition-all z-10'>
        ...
        <div className='absolute bottom-0 left-0 right-0 p-3 flex items-end justify-between text-white'>
          <div>
            {post.interacoes?.quantidadeLike > 0 && (
              <div className='text-sm mb-1 flex gap-1 items-center'>
                <span className='material-symbols-outlined' style={{ fontSize: '1.1rem' }}>thumb_up</span>
                <span>{post.interacoes.quantidadeLike}</span>
              </div>
            )}
            <h4 className='text-sm font-semibold truncate max-w-[140px]'>{post.titulo}</h4>
          </div>
          ...
          {souEu && (
            ...
            >
              {deleting
                ? <span className='material-symbols-outlined animate-spin' style={{ fontSize: '1.3rem' }}>progress_activity</span>
      ...
      {/* media */}
      {isVideo ? (
        <video preload='none' muted loop className='w-full h-full object-cover object-center'>
          <source type={post.media.mimeType} src={mediaSrc} />
        </video>
      ) : (
        <img src={mediaSrc} alt={post.titulo} loading='lazy' className='w-full h-full object-cover object-center' />
      )}
    </Link>
  )
}
