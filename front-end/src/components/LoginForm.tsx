...
function LoginButton() {
	...
	return (
		<button
			className="block button primary w-full mb-2 rounded-full"
			disabled={status.pending}
			type="submit"
		>
			{status.pending ? "..." : "Entrar"}
		</button>
	);
}
...
export default function LoginForm() {
	...
	const makeLoginAttempt = async (form: FormData) => {
		...
		try {
			setLoginError(false);
			const _data = await api.fazerLogin(email, senha);
	...
	return (
		<form action={makeLoginAttempt} className="flex flex-col w-full">
			{loginError && (
				<span className="p-3 text-xs bg-red-100 mb-3 self-start border-1 border-red-300 rounded-md">
			...
			<div className="my-4">
				<LoginButton />
				<Link
					className="block button ghost w-full rounded-full"
					to="/forgot-password"
				>
					Esqueci minha senha
				</Link>
